...
export type OutputMode = 'markdown' | 'json' | 'both'
...
function truncate(
  ...
  return {
    ...
    } of ${text.length} characters — raise max_chars to see the rest]`,
    truncated: true,
  }
}

/** One-line-per-file summary: what was parsed, how, and how fast. */
export function summarise(results: ConvertResult[]): string {
...
export function renderResults(
  ...
  const blocks: string[] = [summarise(results), '']
