...
const outputMode = z
  ...
  .default('markdown')
  .describe(
    "'markdown' for readable text, 'json' for structure plus per-page content, 'both' for structure plus markdown",
  )
...
server.registerTool(
  'doc_parser_health',
  {
    ...
    description:
      'Check the document parser service: status, version, and whether OCR (tesseract) is available on the server. Needs no API key.',
    inputSchema: {},
...
server.registerTool(
  'convert_documents',
  {
    ...
    description:
      'Convert one or more local documents to markdown and structured JSON, with OCR for scanned pages. ' +
    ...
    inputSchema: {
      ...
      ocr: z
        ...
        .default(true)
        .describe(
          'Run OCR on images and image-based PDF pages. Set false to use the embedded text layer only (much faster, empty for scans).',
        ),
      format: z
...
server.registerTool(
  'convert_document_raw',
  {
    ...
    description:
      'Convert exactly one local document via the raw-body endpoint, skipping multipart framing. ' +
...
server.registerTool(
  'inspect_documents',
  {
    ...
    description:
      'Report page counts, detected format, which pages needed OCR, and timing for one or more documents — without pulling the extracted text into context. ' +
